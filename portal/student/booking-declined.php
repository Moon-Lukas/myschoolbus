<?php
    ...
    $query_get_rtps = mysqli_query($connection, "SELECT COUNT(*) AS 'notification' FROM `bookings` WHERE `status`= 'Pending' AND expired='No' AND `user_id`='".$user_login_id."'");
...
<body>
    ...
    <section>
        ...
        <?php
                ...
                $counter = 1;
                //get user information and prepare for display
                $query_get_reviews = mysqli_query($connection, "SELECT u.NRC, u.fullnames, b.reason, b.date_from, b.date_to, b.status, b.car_id FROM users u, bookings b WHERE u.username = b.user_id AND b.status = 'Declined' AND u.username = '".$user_login_id."'");
                echo "<table>
                        <tr>
                            <th>ID</th>
                            <th>NAMES</th>
                ...
                if($counter == 1){
                    echo "<tr><td></td><td></td><td></td><td style='text-align: center;'>The are no declined requests</td><td></td><td></td><td></td></tr>";
                }
                echo "</table>";
