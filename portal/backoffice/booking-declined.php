...
<body>
    ...
    <section>
        ...
        <?php
                ...
                $counter = 1;
                //get user information and prepare for display
                $query_get_reviews = mysqli_query($connection, "SELECT u.NRC, u.fullnames, b.reason, b.date_from, b.date_to, b.status, b.car_id FROM users u, bookings b WHERE u.username = b.user_id AND b.status = 'Declined'");
                echo "<table>
                        <tr>
                            <th>ID</th>
                            <th>NAMES</th>
                ...
                while($row = mysqli_fetch_array($query_get_reviews)){
                    echo "<tr><td>".$row["NRC"]."</td><td><a href='edit-user.php?id=".$row["username"]."' >".$row["fullnames"]."</a></td><td>".$row["reason"]."</td><td>".$row["date_from"]."</td><td>".$row["date_to"]."</td><td>".$row["car_id"]."</td><td>".$row["status"]."</td></tr>";  
                    $index++;
                    $counter = 0;
                }
                if($counter == 1){
