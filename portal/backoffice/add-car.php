<?php
    ...
    if(isset($_POST["signup-button"])){
        ...
        if(!empty($branchID) && !empty($location_id)){
            $query_existing_user = mysqli_query($connection, "SELECT * FROM `branches` WHERE `branch_name` = '".$branchID."'");

            if($row = mysqli_fetch_array($query_existing_user)){
                              
                $error = '<span style="color: red;">Branch with name "'.$branchID.'" already in the system</span>';
            }
            else{                                
                $query_insert_n = "INSERT INTO `branches`(`branch_name`, `location`, `status`) VALUES ('".$branchID."','".$location_id."','".$status_id."')";
    ...
    <section>
        ...
        <div class="search-section">            
                ...
                <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                    <input id="search-id" type="text" name="branch_name" class="inputs" placeholder="Enter Branch.." style="text-align: center;" Required />
                    <input id="search-id" type="text" name="location" class="inputs" placeholder="Enter Branch location.." style="text-align: center;" Required /><br/><br/>
