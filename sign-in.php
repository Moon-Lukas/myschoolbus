    ...
    if(isset($_POST["submit"])){
       ...
       // echo $hashed_key;
        if(!empty($userID) && !empty($passkey)){
            $select_user = mysqli_query($connection,"SELECT * FROM `user` WHERE `username` = '".$userID."' AND `password` ='".$hashed_key."'");

            if(mysqli_num_rows($select_user) > 0){
                session_start();
                $_SESSION["username"] = $userID;
                Header("Location:portal/student/home.php");
            }
            else{
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="sign-in-form-area">
                        ...
                        <form class="sign-in-form" method="post" action="sign-in.php">
